...
const cajaChicaService = {
  ...
  obtenerMovimientos: async (idCajaChica) => {
    const response = await apiClient.get(`/movimientos-cajachica/caja/${idCajaChica}`);
    return response;
  },

  // Obtener movimiento por ID
  async obtenerMovimiento(id) {
    const response = await apiClient.get(`/movimientos-cajachica/${id}`);
    return response.data;
  },

  // Crear nuevo movimiento
  async crearMovimiento(movimiento) {
    const response = await apiClient.post('/movimientos-cajachica', movimiento);
    return response.data;
  },

  // Actualizar movimiento
  async actualizarMovimiento(id, movimiento) {
    const response = await apiClient.put(`/movimientos-cajachica/${id}`, movimiento);
    return response.data;
  },

  // Eliminar movimiento
  async eliminarMovimiento(id) {
    const response = await apiClient.delete(`/movimientos-cajachica/${id}`);
    return response.data;
  },

  // Calcular saldo actual de una caja chica
  async calcularSaldoActual(idCajaChica) {
    const response = await apiClient.get(`/movimientos-cajachica/caja/${idCajaChica}/saldo`);
    return response.data;
  }
};
