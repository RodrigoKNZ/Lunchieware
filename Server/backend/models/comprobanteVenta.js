  ...
  async registrarVenta({ idContrato, tipoComprobante, numeroSerie, numeroComprobante, fechaDocumento, formaPago, medioDePago, detalles, importeImponible, importeImpuesto, importeTotal, saldoAlMomentoDeVenta }) {
    ...
    try {
      ...
      const valuesComprobante = [
        tipoComprobante, numeroSerie, numeroComprobante, fechaDocumento, idContrato, formaPago, medioDePago, importeImponible, importeImpuesto, importeTotal, saldoAlMomentoDeVenta, consumo.idConsumo
      ];
      const resultComprobante = await client.query(queryComprobante, valuesComprobante);
      const comprobante = resultComprobante.rows[0];
      // 3. Crear detalles
      ...
      // 4. Actualizar importes en contrato SOLO si es con cargo en cuenta
      if (formaPago === 'cuenta') {
